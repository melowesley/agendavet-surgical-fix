...
// Get Chat Feedback (Extract text summary, generating state, and actionable buttons)
async function getChatFeedback(cdp) {
    const EXP = `(() => {
        try {
            const cascade = document.getElementById('conversation') || document.getElementById('chat') || document.getElementById('cascade');
            if (!cascade) return { error: 'chat container not found' };

            const isGenerating = !!document.querySelector('[data-tooltip-id="input-send-button-cancel-tooltip"]');
            
            // Extract recent text (last few messages)
            let recentText = '';
            const messages = Array.from(cascade.querySelectorAll('.whitespace-pre-wrap, .markdown'));
            if (messages.length > 0) {
                // Get the last 2-3 messages for context
                const recentMsgs = messages.slice(-3);
                recentText = recentMsgs.map(m => m.innerText).join('\\n---\\n').trim();
            }

            // Extract Actionable Buttons (Run, Accept, Reject, etc)
            const buttons = [];
            const actionKeywords = ['run', 'accept', 'reject', 'approve', 'continue', 'confirm'];
            
            // 1. Look for explicit buttons or divs with button role
            const clickables = Array.from(cascade.querySelectorAll('button, div[role="button"]'));
            
            clickables.forEach((el, index) => {
                const text = (el.innerText || '').trim().toLowerCase();
                if (text && actionKeywords.some(kw => text.includes(kw))) {
                    // It's an action button!
                    // Find a clean action name
                    let actionName = actionKeywords.find(kw => text.includes(kw));
                    if (text.includes('accept all')) actionName = 'accept all';
                    if (text.includes('reject all')) actionName = 'reject all';

                    buttons.push({
                        text: el.innerText.trim(),
                        action: actionName,
                        index: index // We'll use index for remote clicking later
                    });
                }
            });
...
async function main() {
    ...
    try {
        ...
        // Get Chat Feedback
        app.get('/chat-feedback', async (req, res) => {
            if (!cdpConnection) return res.json({ error: 'CDP disconnected', isGenerating: false, recentText: '', buttons: [] });
            const result = await getChatFeedback(cdpConnection);
            res.json(result);
        });

        // Admin endpoint to toggle AUTO_EXECUTE flag (protected by simple token)
        app.post('/admin-toggle', async (req, res) => {
            const token = req.headers['x-admin-token'];
            if (!token || token !== process.env.TELEGRAM_ADMIN_TOKEN) {
            ...
            process.env.TELEGRAM_AUTO_EXECUTE = enable ? 'true' : 'false';
            res.json({ success: true, AUTO_EXECUTE: process.env.TELEGRAM_AUTO_EXECUTE });
        });
