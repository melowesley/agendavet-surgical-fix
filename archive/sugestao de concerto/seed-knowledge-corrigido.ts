...
// Função CRÍTICA: Limpar variáveis (resolve 90% dos erros Windows)
const cleanEnv = (key: string): string => {
  ...
  return value
    ...
    .replace(/[\r\n\t]/gm, '')       // Remove quebras de linha (Windows)
    .replace(/\u0000/g, '')          // Remove null bytes
    .replace(/^["']|["']$/g, '');    // Remove aspas encapsulando
};

// CONFIG SEGURA - Chaves já limpas
const CONFIG = {
...
// ==================== EXECUÇÃO PRINCIPAL ====================
async function executarSeed(agente: 'google' | 'deepseek' | 'kimi') {
  console.log(`
╔═══════════════════════════════════════════════════════════════╗
...
🎯 Próximos passos:
   1. Expandir para múltiplos protocolos
   2. Implementar busca por similaridade
   3. Testar RAG (Retrieval-Augmented Generation)
    `);
  } catch (err: any) {
    console.error(`
╔═══════════════════════════════════════════════════════════════╗
...
🔧 Dicas de troubleshooting:
   1. Execute primeiro: npx tsx agendavet-diagnostico.ts
   2. Verifique .env.local está na raiz do projeto
   3. Confirme que Supabase está acessível
   4. Para Google: chave começa com "AIzaSy"
   5. Para DeepSeek/Kimi: chave começa com "sk-"
    `);
    process.exit(1);
  }
}

// EXECUTAR COM GOOGLE (mais confiável e gratuito)
executarSeed('google').catch(console.error);

// Para testar outros agentes, descomente:
// executarSeed('deepseek').catch(console.error);
// executarSeed('kimi').catch(console.error);
